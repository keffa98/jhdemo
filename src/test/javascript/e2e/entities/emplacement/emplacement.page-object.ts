...
export class EmplacementUpdatePage {
  ...
  nomEplacementInput = element(by.id('field_nomEplacement'));

  async getPageTitle(): Promise<string> {
    return this.pageTitle.getAttribute('jhiTranslate');
  }

  async setNomEplacementInput(nomEplacement: string): Promise<void> {
    await this.nomEplacementInput.sendKeys(nomEplacement);
  }

  async getNomEplacementInput(): Promise<string> {
    return await this.nomEplacementInput.getAttribute('value');
  }
