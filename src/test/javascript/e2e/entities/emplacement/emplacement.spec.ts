...
describe('Emplacement e2e test', () => {
  ...
  it('should load Emplacements', async () => {
    ...
    expect(await emplacementComponentsPage.getTitle()).to.eq('jhdemoApp.emplacement.home.title');
    await browser.wait(
  ...
  it('should load create Emplacement page', async () => {
    ...
    expect(await emplacementUpdatePage.getPageTitle()).to.eq('jhdemoApp.emplacement.home.createOrEditLabel');
  ...
  it('should create and save Emplacements', async () => {
    ...
    await promise.all([emplacementUpdatePage.setNomEplacementInput('nomEplacement')]);

    expect(await emplacementUpdatePage.getNomEplacementInput()).to.eq(
      'nomEplacement',
      'Expected NomEplacement value to be equals to nomEplacement'
    );
  ...
  it('should delete last Emplacement', async () => {
    ...
    expect(await emplacementDeleteDialog.getDialogTitle()).to.eq('jhdemoApp.emplacement.delete.question');
